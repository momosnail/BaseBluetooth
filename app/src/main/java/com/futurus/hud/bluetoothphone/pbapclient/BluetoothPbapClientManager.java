package com.futurus.hud.bluetoothphone.pbapclient;
...
//Phone Book Access Profile 电话本 联系人
public class BluetoothPbapClientManager {
    ...
    private int mConnectStatus = ConnectionState.DISCONNECTED.ordinal();
    ...
    public void initConnect(BluetoothDevice device) {
        ...
        if(mBluetoothPbapClient != null && !getDevice().getAddress().equals(device.getAddress())){
            ...
            mBluetoothPbapClient = new BluetoothPbapClient(device, mHandler);
    ...
    public int getConnectState() {
        Log.d(TAG,"getConnectState  mConnectStatus+0 = "+mConnectStatus);
        if(mBluetoothPbapClient != null && 
            mConnectStatus != mBluetoothPbapClient.getState().ordinal()){
            mConnectStatus = mBluetoothPbapClient.getState().ordinal();
        }
        Log.d(TAG,"getConnectState  mConnectStatus+1 = "+mConnectStatus);
    ...
    public boolean pullVcardList(String folder, byte order, byte searchAttr,
        ...
        return mBluetoothPbapClient.pullVcardListing(folder, order, searchAttr, searchVal,
                maxListCount, listStartOffset);
    }

    public boolean setAuthResponse(String key) {
        return mBluetoothPbapClient.setAuthResponse(key);
    }
