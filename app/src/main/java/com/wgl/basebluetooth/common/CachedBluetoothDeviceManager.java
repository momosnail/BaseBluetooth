...
package com.wgl.basebluetooth.common;
...
public class CachedBluetoothDeviceManager {
    ...
    public synchronized Collection<CachedBluetoothDevice> getCachedDevicesCopy() {//获取缓存设备列表
     ...
     */
    public CachedBluetoothDevice findDevice(BluetoothDevice device) {//通过设备名称查找历史列表是否有 有则返回
	...
	public CachedBluetoothDevice findDeviceByaddr(String addr) {//通过设备地址查找历史列表是否有 有则返回
    ...
    public CachedBluetoothDevice addDevice(LocalBluetoothAdapter adapter,
            ...
            BluetoothDevice device) {//添加一个设备到设备列表
    ...
    public synchronized void clearNonBondedDevices() {//清除没有配对的设备
    ...
    public synchronized void onScanningStateChanged(boolean started) {//重新扫描 移除旧设备
        if (!started) return;
    ...
    public synchronized void onBluetoothStateChanged(int bluetoothState) {
        ...
        if (bluetoothState == BluetoothAdapter.STATE_TURNING_OFF) {
            for (int i = mCachedDevices.size() - 1; i >= 0; i--) {
                CachedBluetoothDevice cachedDevice = mCachedDevices.get(i);
                if (cachedDevice.getBondState() != BluetoothDevice.BOND_BONDED) {//蓝牙即将关闭的时候 清除没有绑定的设备
                    cachedDevice.setVisible(false);
                    ...
                    cachedDevice.clearProfileConnectionState();//已配对的设备 清除连接状态
                }
            }
        }
    }
