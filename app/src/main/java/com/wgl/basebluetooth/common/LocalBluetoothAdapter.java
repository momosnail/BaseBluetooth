...
public final class LocalBluetoothAdapter {
    ...
    public boolean isDiscovering() {
        return mAdapter.isDiscovering();//如果本地蓝牙适配器当前处于设备发现过程中，则返回true ,设备发现是一个重量级的过程。在发现过程中不应尝试与远程蓝牙设备建立新连接，并且现有连接将遇到有限的带宽和高延迟。使用cancelDiscovery()取消正在进行的发现。
    }
    ...
    public void startScanning(boolean force) {
        // Only start if we're not already scanning 仅当我们已经不扫描的时候
   ...
   public void stopScanning() {
    ...
    public synchronized int getBluetoothState() {
        ...
        syncBluetoothState();
		Log.v(TAG, "getBluetoothState mState:" + mState);
        return mState;
    }
    ...
    public void setBluetoothEnabled(boolean enabled) {
		Log.v(TAG, "setBluetoothEnabled mAdapter:" + mAdapter);
